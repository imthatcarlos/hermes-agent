...
metadata: {"clawdbot":{"emoji":"💰","requires":{"bins":["solana","spl-token","curl","jq","node"]}}}
---
...
The following environment variables are used (at least one keypair source is required):

| Variable | Description |
|----------|-------------|
| `SOLANA_KEYPAIR` | JSON array of secret key bytes (preferred for secrets management) |
| `SOLANA_KEYPAIR_PATH` | Path to wallet keypair JSON file (fallback) |
| `JUPITER_API_KEY` | Jupiter API key for authenticated requests (avoids platform fees, required for Token2022/pump.fun tokens) |

**Keypair Priority:** `SOLANA_KEYPAIR` env → `--keypair` arg → `SOLANA_KEYPAIR_PATH` env

**Note:** These should be pre-configured in your environment. The jupiter-swap.mjs script automatically detects which source to use.
